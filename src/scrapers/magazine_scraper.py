...
class MagazineScraper:
    ...
    def collect(self, url, no_browser=False):
        ...
        any_blocked = False
        last_error = None
        surfsky_original_attempted = False
        surfsky_terminal_error = False
        ...
        # v14.14: em cloud o Chromium local da Railway e as URLs alternativas
        # costumam ser bloqueados pelo Magalu. Se Surfsky estiver configurado,
        # tenta a PDP ORIGINAL imediatamente depois do HTTP barato. Isso evita
        # gastar vários timeouts antes de chegar ao caminho que já funciona.
        if not no_browser:
            try:
                from .browser_scraper import BrowserScraper
                surfsky_configured_early = BrowserScraper().surfsky_configured()
            except Exception:
                surfsky_configured_early = False

            if surfsky_configured_early:
                surfsky_original_attempted = True
                try:
                    result, error, blocked = self._try_surfsky_product(
                        url, url, "MAGALU_SURFSKY_CLOUD"
                    )
        ...
        # 2) Mesmo link em Chromium/Playwright. Só vale como fallback quando
        # Surfsky não está configurado; caso contrário ele apenas acrescenta
        # timeout de datacenter antes da coleta residencial.
        if not no_browser and not surfsky_original_attempted:
            try:
        ...
        if not no_browser:
            ...
            if surfsky_configured and not surfsky_terminal_error:
                if surfsky_original_attempted:
                    # Já tentamos a PDP original no caminho rápido. Se ela não
                    # funcionou, tenta no máximo a principal equivalente oficial.
                    surfsky_candidates = alternative_candidates[:1]
