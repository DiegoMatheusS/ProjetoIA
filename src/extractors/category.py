...
ALIASES = {
    ...
    "CAMERA DE SEGURANCA": "CAMERA_SEGURANCA",
    "LAMPADA INTELIGENTE": "LAMPADA_INTELIGENTE",
    "TOMADA INTELIGENTE": "TOMADA_INTELIGENTE",
    "FECHADURA INTELIGENTE": "FECHADURA_INTELIGENTE",
    "E READER": "E_READER",
    "DRONE": "DRONE",
    "CAMERA DE ACAO": "CAMERA_ACAO",
    "SOUNDBAR": "SOUNDBAR",
...
def detect_category(text: str, forced: str | None = None):
    ...
    headline_rules = [
        ...
        ("MEMORIA_RAM", [
            r"^\s*memoria(?:\s+ram)?\b",
            r"^\s*kit\s+(?:de\s+)?(?:memoria|ram)\b",
            r"^\s*ram\b",
            r"\b(?:4|8|16|24|32|48|64|96|128)\s*gb\b.{0,45}\bddr[345]\b.{0,35}\b(?:\d{4,5})\s*mhz\b",
            r"\bddr[345]\b.{0,45}\b(?:4|8|16|24|32|48|64|96|128)\s*gb\b.{0,35}\b(?:\d{4,5})\s*mhz\b",
            r"\b(?:so[- ]?dimm|udimm|dimm)\b.{0,45}\bddr[345]\b",
        ]),
    ...
    rules = [
        ...
        ("ROBO_ASPIRADOR", [r"\brob[oô]\s+aspirador\b", r"\baspirador\s+rob[oô]\b", r"\brobot\s+vacuum\b"]),
        ("ASPIRADOR_PO", [r"\baspirador(?:\s+de\s+p[oó])?\b", r"\bvacuum\s+cleaner\b"]),
        ("CAMERA_SEGURANCA", [r"\bc[aâ]mera\b.{0,25}\b(?:seguran[cç]a|vigil[aâ]ncia|ip|cftv)\b"]),
        ("CAMERA_ACAO", [r"\bc[aâ]mera\s+de\s+a[cç][aã]o\b", r"\baction\s+camera\b", r"\bgopro\b"]),
        ("SMART_SPEAKER", [r"\bsmart\s+speaker\b", r"\b(?:echo\s+(?:dot|show|pop)|homepod|nest\s+audio)\b"]),
        ("LAMPADA_INTELIGENTE", [r"\bl[aâ]mpada\s+inteligente\b", r"\bsmart\s+bulb\b"]),
        ("TOMADA_INTELIGENTE", [r"\btomada\s+inteligente\b", r"\bsmart\s+plug\b"]),
        ("FECHADURA_INTELIGENTE", [r"\bfechadura\s+(?:digital|inteligente|smart)\b", r"\bsmart\s+lock\b"]),
        ("E_READER", [r"\be[- ]?reader\b", r"\bkindle\b", r"\bkobo\b"]),
        ("DRONE", [r"\bdrone\b", r"\bquadric[oó]ptero\b"]),
        ("SOUNDBAR", [r"\bsoundbar\b"]),
        ("HOME_THEATER", [r"\bhome\s+theater\b"]),
        ("AIR_FRYER", [r"\bair\s*fryer\b", r"\bfritadeira\s+el[eé]trica\b"]),
        ("CAFETEIRA", [r"\bcafeteira\b", r"\bcoffee\s+maker\b"]),
        ("LIQUIDIFICADOR", [r"\bliquidificador\b", r"\bblender\b"]),
        ("CLIMATIZADOR", [r"\bclimatizador\b", r"\bair\s+cooler\s+port[aá]til\b"]),
        ("VENTILADOR", [r"\bventilador\b.{0,25}\b(?:mesa|coluna|torre|parede|port[aá]til)?\b"]),
        ...
        ("KIT_ARDUINO_ROBOTICA", [r"\bkit\b.{0,30}\b(?:arduino|rob[oó]tica)\b", r"\bkit\s+de\s+rob[oó]tica\b"]),
        ...
        ("MEMORIA_RAM", [
            r"memoria\s+ram",
            r"\b(?:ddr[345])\b.{0,40}\b(?:4|8|16|24|32|48|64|96|128)\s*gb\b",
            r"\b(?:4|8|16|24|32|48|64|96|128)\s*gb\b.{0,40}\b(?:ddr[345])\b",
            r"\b(?:so[- ]?dimm|udimm|dimm)\b.{0,40}\bddr[345]\b",
        ]),
