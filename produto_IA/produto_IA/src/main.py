...
def build_result(raw, forced_category=None):
    ...
    return {
        ...
        "marketplace": {
            ...
            "surfskyConfigurado": bool(os.getenv("SURFSKY_TOKEN", "").strip()),
            "surfskyApiUrl": os.getenv("SURFSKY_API_URL", "https://api-public.surfsky.io") if os.getenv("SURFSKY_TOKEN", "").strip() else None,
            "surfskyPais": (os.getenv("SURFSKY_PROXY_COUNTRY", "br") or "br").lower() if os.getenv("SURFSKY_TOKEN", "").strip() else None,
            "surfskyProxyTier": os.getenv("SURFSKY_PROXY_TIER") or None if os.getenv("SURFSKY_TOKEN", "").strip() else None,
            "browserlessConfigurado": bool(os.getenv("BROWSERLESS_TOKEN", "").strip()),
        },
        "fonte": raw.get("source"),
        "erro": result_error,
    }
...
def main():
    ...
    if args.enrich or auto_enrich or mandatory_missing_enrichment:
        result = apply_enrichment(result, auto_mode=not bool(args.enrich))
