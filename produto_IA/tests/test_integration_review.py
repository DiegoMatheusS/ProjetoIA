...
from src.technical_ai.providers import OpenAIProvider, TechnicalAIProviderError
...
def test_ai_does_not_parse_internal_thoughts():
    payload = {'output': [{'type': 'reasoning', 'summary': [{'text': 'Socket: AM4'}]}, {'type': 'message', 'content': [{'type': 'output_text', 'text': 'Socket: AM5'}]}]}
    assert OpenAIProvider._response_text(payload) == 'Socket: AM5'
...
def test_truncated_ai_response_is_not_applied(monkeypatch):
    ...
    class Response:
        status_code = 200
        def json(self):
            return {'status': 'incomplete', 'incomplete_details': {'reason': 'max_output_tokens'}, 'output_text': 'Socket: AM5'}
    class Session:
        def post(self, *args, **kwargs):
            return Response()
    with pytest.raises(TechnicalAIProviderError, match='interrompida'):
        OpenAIProvider(session=Session()).enrich('CPU')
